...
def activity_countrate(file_name_s, sheetname_s, Em_probalility, activity):
    ...
    efficency_mass_mat = efficency_mass.values
    efficency_mass_mat_em = efficency_mass_mat * Em_probalility
    Activity_optimized = np.linalg.lstsq(np.linalg.inv(efficency_mass_mat_em), activity)[0] 
    return Activity_optimized
...
for key in parameter:
    Activity_all[key] = activity_countrate(file_name_s = file_name, 
                                            sheetname_s = key, 
                                            Em_probalility = Eimission_probability[parameter[key][0]].values, 
                                            activity = Measured_data["Lab_activity"].values)
...
writer = pd.ExcelWriter(working_directory+"/Activity_all_data_to_countrate.xlsx")
