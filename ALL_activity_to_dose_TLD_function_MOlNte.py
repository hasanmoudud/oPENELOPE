...
for nuclide in nuclides:
    ...
    Dose_activity_optimized = np.linalg.lstsq(np.linalg.inv(Effi_dose_microSv_s_mat), Measured_activity)[0]
    Activity_all[nuclide] = Dose_activity_optimized


Activity_all_data= pd.DataFrame.from_dict(Activity_all)
writer = pd.ExcelWriter(working_directory+"/Dose_all_data_optimized_insitu.xlsx")
